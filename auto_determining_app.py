...
def main() -> None:
    st.set_page_config(page_title="Auto Determining", layout="wide")
    _ensure_data_dirs()
    st.title("Auto Determining")
    ...
    audio_path = _save_upload(uploaded) if uploaded is not None else _load_local_path(local_path_text)
    ...
    edited_rows = st.data_editor(events_to_editor_rows(beatmap["events"]), hide_index=True, num_rows="dynamic", use_container_width=True)
    beatmap["events"] = events_from_editor(edited_rows, beatmap["source"]["duration_sec"])
    validate_beatmap(beatmap)

    chart_data = pd.DataFrame(
        {"time_sec": [event["time_sec"] for event in beatmap["events"]], "strength": [event["strength"] for event in beatmap["events"]]}
    )
    ...
    st.download_button("Download JSON", payload, file_name=_beatmap_download_name(audio_path), mime="application/json")
